...
class ModelCatalogCategory extends Model {
	...
	public function editCategory($category_id, $data) {
		...
		if (isset($data['image'])) {
            $tmp_images[] = "image = '" . $this->db->escape(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . "'";
			//$this->db->query("UPDATE " . DB_PREFIX . "category SET image = '" . $this->db->escape(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . "' WHERE category_id = '" . (int)$category_id . "'");
		}
		if (isset($data['image_banner'])) {
            $tmp_images[] = "image_banner = '" . $this->db->escape(html_entity_decode($data['image_banner'], ENT_QUOTES, 'UTF-8')) . "'";
		}

        if (count($tmp_images) >0){
            $this->db->query("UPDATE " . DB_PREFIX . "category SET ". implode(', ', $tmp_images) ." WHERE category_id = '" . (int)$category_id . "'");
