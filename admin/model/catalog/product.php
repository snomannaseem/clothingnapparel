...
class ModelCatalogProduct extends Model {
	...
	public function editProduct($product_id, $data) {
		...
		if (isset($data['product_image'])) {
			foreach ($data['product_image'] as $product_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape(html_entity_decode($product_image['image'], ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . (int)$product_image['sort_order'] . "', color_id = '" . (int)$product_image['color_id'] . "'");
			}
		}
