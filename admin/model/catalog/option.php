...
class ModelCatalogOption extends Model {
		...
		if (isset($data['option_value'])) {
			foreach ($data['option_value'] as $option_value) {
				...
				foreach ($option_value['option_value_description'] as $language_id => $option_value_description) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "option_value_description SET option_value_id = '" . (int)$option_value_id . "', language_id = '" . (int)$language_id . "', option_id = '" . (int)$option_id . "', name = '" . $this->db->escape($option_value_description['name']) . "', op_desc1 = '" . $this->db->escape($option_value_description['op_desc1']) . "'");
				}
			}
		}
	}
		...
		foreach ($option_value_query->rows as $option_value) {
			...
			foreach ($option_value_description_query->rows as $option_value_description) {
				$option_value_description_data[$option_value_description['language_id']] = array(
                                                                                                  ...
                                                                                                  'op_desc1' => $option_value_description['op_desc1']
                                                                                                );
    ...
    */
    ...
    public function getAllColorOptions() {
		$data = array();
		
		$query = $this->db->query("SELECT option_value_id AS color_id, option_value_description.name AS color_name FROM " . DB_PREFIX . "option_value_description
            INNER JOIN " . DB_PREFIX . "option_description ON " . DB_PREFIX . "option_value_description.option_id = " . DB_PREFIX . "option_description.option_id
            WHERE " . DB_PREFIX . "option_description.name = 'Color'");
    ...
    public function getAllSizeOptions() {
		$data = array();
		
		$query = $this->db->query("SELECT option_value_id AS size_id, option_value_description.name AS size_name FROM " . DB_PREFIX . "option_value_description
            INNER JOIN " . DB_PREFIX . "option_description ON " . DB_PREFIX . "option_value_description.option_id = " . DB_PREFIX . "option_description.option_id
            WHERE " . DB_PREFIX . "option_description.name = 'Size'");
