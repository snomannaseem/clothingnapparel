...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'special'          => $query->row['special'] * 0.95,
				...
				'default_color_id' => $query->row['default_color_id']
			);
		} else {
			return false;
		}
	...
	public function getProductOptions($product_id, $option_description_name = "", $color_id = 0) {
		$product_option_data = array();
        $tmp_option_desc_filter = "";
        if (!empty($option_description_name)){
            $tmp_option_desc_filter = " AND od.name = '${option_description_name}'";
        }
        
        $tmp_sub_query = "";
        if ($color_id > 0){
            $tmp_sub_query = " AND ovd.option_value_id in ( SELECT size_id FROM ocean_inventory WHERE product_id = ${product_id} AND color_id = ${color_id} AND quantity > 0) ";
        }
		...
		foreach ($product_option_query->rows as $product_option) {
			if ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image') {
                ...
                $tmp_sql = "SELECT * FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_id = '" . (int)$product_id . "' AND pov.product_option_id = '" . (int)$product_option['product_option_id'] . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ${tmp_sub_query} ORDER BY ov.sort_order";
				$product_option_value_query = $this->db->query($tmp_sql);
				...
				foreach ($product_option_value_query->rows as $product_option_value) {
					$product_option_value_data[] = array(
						...
						'op_desc1'                => $product_option_value['op_desc1'],
	...
	public function getProductImages($product_id, $color_id=0) {
        if ($color_id > 0){
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_image WHERE product_id = '" . (int)$product_id . "' AND color_id = '" . (int)$color_id . "' ORDER BY sort_order ASC");
        } else{
	...
	public function getProductSizesByColor($product_id, $color_id=0) {
        
        $sql = "SELECT * FROM vw_ocean_product_sizes WHERE product_id = '" . (int)$product_id . "'";
        if ($color_id > 0){
            $sql  .= " AND color_id = '" . (int)$color_id . "' AND quantity > 0 ";
            //$sql  .= " AND color_id = '" . (int)$color_id . "'";
        }
	...
	public function getProductDefaultColor($product_id) {
