...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		if ($product_info) {
			...
			$this->document->setTitle($product_info['title']);
			...
			$tmp_default_color_id = $product_info['default_color_id'];
            
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id'], 'Color') as $option) { 
            ...
            foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id'], '', $tmp_default_color_id) as $option) { 
				if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'checkbox' || $option['type'] == 'image') { 
					$option_value_data = array();
            ...
            $results = $this->model_catalog_product->getProductImages($this->request->get['product_id'], $tmp_default_color_id);
    ...
    public function getImages() {
		...
		// if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			// if (!isset($json['error'])) {
				// $this->model_catalog_review->addReview($this->request->get['product_id'], $this->request->get);
        ...
        $results_images = $this->model_catalog_product->getProductImages($this->request->get['product_id'], $this->request->get['color_id']);
        ...
        foreach ($results_sizes as $result) {
            $tmp_size_name = $result['size_name'];
            if ($result['size_price'] > 0 ){
                $tmp_size_name .= "    (". $result['size_price_prefix']. "$".  number_format($result['size_price'],2) . ")";
            }
            
            $json['sizes'][] = array(
                'size_id' => $result['size_id'],
                'size_name' => $tmp_size_name
            );
        }
